...
- [ ] **soft delete 적용**: 거래처/상품/가격등급/명세서/입금에 `is_active` 컬럼 추가, 삭제 → 비활성화. 휴지통 페이지는 만들지 않고 Supabase SQL로 직접 복구
- [ ] **RLS 기본 설정**: 인증된 사용자만 데이터 접근
- [ ] **권한 시스템**: `user_roles` 테이블 (admin/staff). 관리자 2명(본인+남편), 직원 개별 계정. 향후 다른 역할 추가 가능하도록 텍스트 컬럼 설계. 직원 추가는 Supabase 화면에서 직접
- [ ] **RLS 역할별 세분화**:
  - 관리자: 모든 기능
  - 직원: 발송 페이지만 (송장번호 입력, 발송 안내 카톡 복사). 가격 정보 노출 안 됨
...
2일차 (새기능) ────────── ✅ 완료 (대시보드, 엑셀 내보내기)
    │
3일차 (DB보안+권한) ──── 진행 중 (트리거 ✅, soft delete/RLS/권한 남음)
    │
4일차 (최적화) ────────── 모바일 UI, 성능
```
